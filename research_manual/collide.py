...
def build_entities(rows, min_hotkeys, meta_path):
    ...
    named = sorted(
        (coldkey for coldkey, count in sizes.items()
         if count >= min_hotkeys and coldkey in seen),
        key=lambda coldkey: (-sizes[coldkey], coldkey),
    )
    named_set = set(named)
    of = {
        coldkey: coldkey if coldkey in named_set else MERGED
        for coldkey in seen
    }
    names = list(named)
...
def prefix(coldkey):
    if coldkey == MERGED or coldkey.startswith("our_"):
        return coldkey
...
def main():
    ...
    of, names, sizes, members = build_entities(
        rows, args.min_hotkeys, args.metagraph
    )
    ...
    for name in names:
        n_hotkeys = sum(sizes[ck] for ck in members[name])
