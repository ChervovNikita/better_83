...
//
// v14 = v13 plus a cheaper scoring rule on the SWAP path.
//
// Step census on a losing n=899 instance: 3.8M steps, of which 3.25M are plateau
// swaps and only 279k are adds. So the swap path is ~95% of the work, and it pays
// the same price as the add path: pick() scores every candidate by |N(v) cap cand|,
// which is W=15 popcount-ANDs per candidate. That scoring is worth it when choosing
// what to ADD (it decides how many options survive), but a plateau swap does not
// change |C| at all — it only shifts which basin we are in.
//
// So: keep the informed rule where it decides something, use LSCC's cheap rule
// (degree, oldest wins ties) where it does not. LSCC scores swap candidates
// statically for exactly this reason.
//
// v13 = v7 plus a guard that skips bucket maintenance that cannot change anything.
//
// v7 calls refresh() on every non-neighbour of a moved vertex. But the buckets only
// hold slack 0 and slack 1, and slack moves by exactly one per event, so a vertex
// going 5->6 (or 6->5) can never enter or leave a bucket. At density 0.9 with a
// clique of ~60, the typical vertex misses ~6 clique members, so the overwhelming
// majority of those refresh calls are provably no-ops.
//
//   after ++slack: only slack<=2 can matter (0->1 moves buckets, 1->2 leaves one)
//   after --slack: only slack<=1 can matter (1->0 moves buckets, 2->1 enters one)
//
...
struct Search {
    ...
    void add(int v, bool free_neighbours) {
        ...
        ++slack[v];
        for_non_neighbours(g, v, [&](int u) { if (++slack[u] <= 2) refresh(u); });
        refresh(v);
        if (free_neighbours)
    ...
    void drop(int v) {
        ...
        conf[v] = 0;
        --slack[v];
        for_non_neighbours(g, v, [&](int u) { if (--slack[u] <= 1) refresh(u); });
        refresh(v);
    }
    ...
    // Cheap rule for the plateau path: no set-overlap scoring, just degree with an
    // age tie-break, sampled BMS-style.
    int pick_cheap(const std::vector<int> &cand) {
        int m = (int)cand.size();
        ...
        for (int i = 0; i < tries; ++i) {
            ...
            if (bestv < 0 || g.deg[v] > g.deg[bestv] ||
                (g.deg[v] == g.deg[bestv] && age[v] < age[bestv]))
                bestv = v;
        }
        return bestv;
    }
