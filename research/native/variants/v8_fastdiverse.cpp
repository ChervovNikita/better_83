...
int sn83_solve(const uint8_t *adj, int n, double time_limit, uint64_t seed,
    ...
    // v8 = v7's incremental candidate sets + v6's per-thread parameter spread.
    // The two address different halves of the same finding: the misses are
    // reachable, and we fail to reach them. v7 buys more tries per second; the
    // parameter spread makes those tries less alike.
