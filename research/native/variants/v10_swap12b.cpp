...
struct Search {
    ...
    std::vector<u64> inCbits;   // C as a bitset: makes "who blocks v" one AND
    std::vector<int> gHead, gNext, touched;         // blocker -> list, array-based
    ...
    Search(const Graph &gg, Params pp, u64 seed)
        : g(gg), p(pp), rng(seed), slack(gg.n, 0), inC(gg.n, 0), conf(gg.n, 1),
          age(gg.n, 0), mask(gg.W, 0ULL), inCbits(gg.W, 0ULL),
          gHead(gg.n, -1), gNext(gg.n, -1) {
        C.reserve(gg.n);
        addBuf.reserve(gg.n);
    ...
    void clear() {
        ...
        C.clear();
        k = 0;
        std::fill(inCbits.begin(), inCbits.end(), 0ULL);
    ...
    bool try_swap12() {
        ...
        for (int v : cand1.items) {
            ...
            int u = blocker(v);
            if (u < 0) continue;
            if (gHead[u] < 0) touched.push_back(u);
            gNext[v] = gHead[u];
            gHead[u] = v;
        }
        int du = -1, da = -1, db = -1;
        for (int u : touched) {
            for (int a = gHead[u]; a >= 0 && du < 0; a = gNext[a])
                for (int b = gNext[a]; b >= 0; b = gNext[b])
                    if (g.adj(a, b)) { du = u; da = a; db = b; break; }
            if (du >= 0) break;
        }
        for (int u : touched) {                     // reset the heads we used
            for (int a = gHead[u]; a >= 0;) { int nx = gNext[a]; gNext[a] = -1; a = nx; }
            gHead[u] = -1;
        }
    ...
    int blocker(int v) {
        const u64 *co = g.corow(v);
        for (int w = 0; w < g.W; ++w) {
            u64 m = co[w] & inCbits[w];
            if (m) return (w << 6) | __builtin_ctzll(m);
        }
        return -1;
    }
