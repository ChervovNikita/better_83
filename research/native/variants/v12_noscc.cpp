...
//
// v12 ABLATION: strong configuration checking REMOVED (conf is always 1).
//
// Testing the negation of something currently believed load-bearing. SCC was added
// early, when the search ran ~15k steps/s and a plateau cycle was fatal; v7 now runs
// 2-4x faster and perturbs far more often, so the restriction may have stopped
// paying for itself. If parity is unchanged, SCC is dead weight and its per-move
// bookkeeping (touching every neighbour on an add) can go, which is another speedup.
//
...
struct Search {
    ...
    void add(int v, bool free_neighbours) {
        ...
        refresh(v);
        (void)free_neighbours;   // SCC disabled in this ablation
    }
    ...
    void run(Clock::time_point deadline, Shared &sh) {
        ...
        for (;;) {
            ...
            swapBuf.clear();
            if (addBuf.empty())
                swapBuf.assign(cand1.items.begin(), cand1.items.end());
