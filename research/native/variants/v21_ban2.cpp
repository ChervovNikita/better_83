...
//
// v21 = v20 with a much SMALLER ban set, which is the fix for a design error.
//
// The diversity term is 1/(miners returning your EXACT vertex set). Set distance is
// irrelevant — a clique differing by one vertex counts as fully distinct. v20
// optimised for minimum overlap and therefore banned a third of the incumbent, which
// made the re-search so hard that the phase succeeded on only **12% of tasks**
// (measured: 14 of 120 answers changed). The null result was about feasibility in
// the time given, not about the idea.
//
// Banning 1-3 vertices still GUARANTEES a different set if a k-clique is found
// without them, is far easier to satisfy, and still biases towards cliques the
// greedy region does not hand you — which is what the field's popular optima are.
//
// Also relevant: the field's own distinct optima overlap only ~48% on average (min
// overlap ~21%), so genuinely different maximum cliques do exist here; the problem
// was reaching one, not the absence of targets.
//
// SN83_BANK = how many incumbent vertices to ban (default 2).
//
// v20 = v7 plus a final DIVERSIFICATION phase that hunts for a DISTANT clique.
//
// Why the previous two attempts failed. v18/v19 chose among the optima the search
// naturally passes through, and v15 cold-restarted the same search — but all of
// those land in the same "findable" region, which is exactly the region every rival
// solver also lands in. Measured: picking the rarest optimum our own chains found is
// no better than picking one at random (diversity 0.751 vs 0.758).
//
// So do not choose differently — SEARCH differently. Once the best size k is known,
// spend the last slice of the budget looking for another k-clique while a random
// third of the incumbent's vertices are BANNED outright. A k-clique found without
// them cannot be the same clique, and is reached through a different basin, so it is
// less likely to be the one 30 other miners hand in.
//
// Size is unchanged (we only accept a replacement at the same k), so the optimality
// term is untouched; only the diversity term moves. SN83_DIVERSIFY is the fraction
// of the deadline given to the phase, in percent (0 disables, default 25).
//
...
struct Search {
    ...
    std::vector<char> banned;   // vertices the diversification phase may not use
    ...
    Search(const Graph &gg, Params pp, u64 seed)
        : g(gg), p(pp), rng(seed), slack(gg.n, 0), inC(gg.n, 0), conf(gg.n, 1),
          age(gg.n, 0), mask(gg.W, 0ULL), banned(gg.n, 0) {
        C.reserve(gg.n);
        addBuf.reserve(gg.n);
    ...
    void construct() {
        for (;;) {
            ...
            for (int v : cand0.items)
                if (!banned[v]) addBuf.push_back(v);
    ...
    void run(Clock::time_point deadline, Shared &sh) {
        ...
        for (;;) {
            ...
            addBuf.clear();
            for (int v : cand0.items)
                if (conf[v] && !banned[v]) addBuf.push_back(v);
            swapBuf.clear();
            if (addBuf.empty())
                for (int v : cand1.items)
                    if (conf[v] && !banned[v]) swapBuf.push_back(v);
    ...
    // Hunt for a clique of the SAME size that avoids part of the incumbent.
    // `alt` keeps the best find, measured by how little it overlaps `best`.
    std::vector<int> alt;
    int alt_overlap = 1 << 30;
    int bank = 2;                    // how many incumbent vertices to ban per attempt
    long long alt_hits = 0;          // how often the phase actually produced one

    void diversify(Clock::time_point deadline, Shared &sh) {
        int target = (int)best.size();
        if (target < 2) return;
        std::vector<char> inBest(g.n, 0);
        for (int v : best) inBest[v] = 1;
        while (Clock::now() < deadline) {
            std::fill(banned.begin(), banned.end(), 0);
            int nban = bank;
            if (nban < 1) nban = 1;
            if (nban > target - 1) nban = target - 1;
            for (int i = 0; i < nban; ++i) banned[best[rnd(target)]] = 1;
            seed_from(rnd(g.n));
            while (banned[C.empty() ? 0 : C[0]] && k == 1) seed_from(rnd(g.n));
            construct();
            for (int guard = 0; guard < 20000 && (int)C.size() < target; ++guard) {
                if ((guard & 63) == 0 && Clock::now() >= deadline) break;
                ++step;
                addBuf.clear();
                for (int v : cand0.items)
                    if (conf[v] && !banned[v]) addBuf.push_back(v);
                if (!addBuf.empty()) { add(pick(addBuf), true); continue; }
                swapBuf.clear();
                for (int v : cand1.items)
                    if (conf[v] && !banned[v]) swapBuf.push_back(v);
                if (!swapBuf.empty()) {
            ...
            if ((int)C.size() >= target) {
                // Any different set earns the full diversity term, so accept the
                // first success rather than hunting for maximum distance.
                int ov = 0;
                for (int v : C) ov += inBest[v];
                ++alt_hits;
                if (ov < alt_overlap) { alt_overlap = ov; alt = C; }
            }
        }
        std::fill(banned.begin(), banned.end(), 0);
    }
...
int sn83_solve(const uint8_t *adj, int n, double time_limit, uint64_t seed,
    ...
    int divpct = 25;
    if (const char *e = getenv("SN83_DIVERSIFY")) divpct = atoi(e);
    int bank = 2;
    if (const char *e = getenv("SN83_BANK")) bank = atoi(e);
    Shared sh;
    std::vector<std::thread> pool;
    std::vector<std::vector<int>> locals(n_threads);
    std::vector<std::array<long long, 5>> stats(n_threads, {0, 0, 0, 0, 0});
    std::vector<std::vector<int>> alts(n_threads);
    std::vector<int> alt_ov(n_threads, 1 << 30);
    std::vector<long long> alt_hits_v(n_threads, 0);
    ...
    for (int t = 0; t < n_threads; ++t) {
        pool.emplace_back([&, t] {
            ...
            if (divpct > 0) {
                auto now = Clock::now();
                split = now + ((deadline - now) * (100 - divpct)) / 100;
            }
            s.run(split, sh);
            if (divpct > 0 && (int)s.best.size() >= sh.best.load(std::memory_order_relaxed))
                s.diversify(deadline, sh);
            locals[t] = s.best;
            alts[t] = s.alt;
            alt_ov[t] = s.alt_overlap;
            alt_hits_v[t] = s.alt_hits;
    ...
    if (getenv("SN83_DEBUG")) {
        long long hits = 0;
        for (int t = 0; t < n_threads; ++t) hits += alt_hits_v[t];
        fprintf(stderr, "  diversification: %lld alternative cliques found\n", hits);
        for (int t = 0; t < n_threads; ++t)
    ...
    // Prefer an equally large clique that shares the fewest vertices with the one
    // the search converged on. Same size, different region.
    int bestov = 1 << 30;
    for (int t = 0; t < n_threads; ++t)
        if (alts[t].size() == best.size() && alt_ov[t] < bestov) {
            bestov = alt_ov[t];
            best = alts[t];
        }
