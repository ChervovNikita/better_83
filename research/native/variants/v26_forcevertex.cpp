...
struct Search {
    ...
    Search(const Graph &gg, Params pp, u64 seed)
        : g(gg), p(pp), rng(seed), slack(gg.n, 0), inC(gg.n, 0), conf(gg.n, 1),
          age(gg.n, 0), mask(gg.W, 0ULL), seen(4096, 0) {
        C.reserve(gg.n);
        addBuf.reserve(gg.n);
...
// Build the subgraph induced on N(v). Every clique of G[N(v)], plus v, is a clique
// of G containing v -- and the maximum cliques of G that contain v are EXACTLY the
// maximum cliques of this subgraph plus v. So pinning a vertex is a change of search
// SPACE, not a perturbation of the search: a chain pinned to v cannot fall back into
// the unconstrained attractor. `map` carries subgraph index -> original index.
static void induced_on_neighbours(const Graph &g, int v, Graph &sub,
                                  std::vector<int> &map) {
    map.clear();
    for (int u = 0; u < g.n; ++u)
        if (u != v && g.adj(v, u)) map.push_back(u);
    int m = (int)map.size();
    sub.n = m;
    sub.W = (m + 63) / 64;
    sub.bits.assign((size_t)m * sub.W, 0ULL);
    sub.co.assign((size_t)m * sub.W, 0ULL);
    sub.deg.assign(m, 0);
    for (int i = 0; i < m; ++i) {
        int d = 0;
        for (int j = 0; j < m; ++j) {
            if (i == j) continue;
            if (g.adj(map[i], map[j])) {
                sub.bits[(size_t)i * sub.W + (j >> 6)] |= 1ULL << (j & 63); ++d;
            } else {
                sub.co[(size_t)i * sub.W + (j >> 6)] |= 1ULL << (j & 63);
            }
        }
        sub.deg[i] = d;
    }
...
int sn83_solve_many(const uint8_t *adj, int n, double time_limit, uint64_t seed,
    ...
    // Harvest phase. v25 seeded EVERY chain from a clique the search had already
    // converged to and then walked the plateau, so the pool was the plateau-connected
    // component of one attractor. Maximum cliques here are isolated points, so that
    // walk provably cannot leave the component -- which is why 81.8% of the maxima we
    // miss are sole-held by some other miner. SN83_PIN gives a chain its own vertex to
    // build around instead.
    //
    //   0  off: exactly v25 (the control)
    //   1  pin a uniformly random vertex
    //   2  pin a vertex NOT in the incumbent best clique
    //   3  pin the lowest-degree vertex that could still be in a max clique
    //
    // SN83_PINPCT is how many of the chains are pinned; the rest stay unpinned so
    // quality never depends on a pin being satisfiable.
    int pinmode = 0, pinpct = 75;
    if (const char *e = getenv("SN83_PIN")) pinmode = atoi(e);
    if (const char *e = getenv("SN83_PINPCT")) pinpct = atoi(e);

    std::vector<int> incumbent = sh.clique;
    for (auto &l : locals) if ((int)l.size() > (int)incumbent.size()) incumbent = l;
    std::vector<char> in_best(n, 0);
    for (int v : incumbent) in_best[v] = 1;

    // Candidate pins, ordered by policy. A vertex can only be in a max clique if it
    // has at least best_size-1 neighbours, so anything below that is skipped -- a
    // pinned chain that cannot reach best_size is a wasted chain.
    std::vector<int> cands;
    for (int v = 0; v < n; ++v) {
        if (g.deg[v] < best_size - 1) continue;
        if (pinmode == 2 && in_best[v]) continue;
        cands.push_back(v);
    }
    if (pinmode == 3)
        std::sort(cands.begin(), cands.end(),
                  [&](int a, int b) { return g.deg[a] < g.deg[b]; });
    else if (pinmode == 1 || pinmode == 2) {
        u64 st = seed * 0x2545F4914F6CDD1DULL + 0x9e3779b97f4a7c15ULL;
        for (size_t i = cands.size(); i > 1; --i) {
            st ^= st << 13; st ^= st >> 7; st ^= st << 17;
            std::swap(cands[i - 1], cands[st % i]);
        }
    }
    ...
    for (int t = 0; t < n_threads; ++t) {
        pool.emplace_back([&, t] {
            bool pinned = pinmode != 0 && !cands.empty()
                          && (t * 100) / n_threads < pinpct;
            if (pinned) {
                int v = cands[(size_t)t % cands.size()];
                Graph sub;
                std::vector<int> map;
                induced_on_neighbours(g, v, sub, map);
                if (sub.n < best_size - 1) return;
                // A LOCAL Shared. Passing the global `sh` here was a real bug: this
                // search runs on the subgraph's own vertex numbering, and record()
                // would write those indices into sh.clique, corrupting the incumbent
                // that unpinned chains seed from and the empty-pool fallback returns.
                Shared subsh;
                Search s(sub, p, seed + 0x51ed270b7f4a7c15ULL * (u64)(t + 7));
                s.pool_cap = per_thread;
                s.target_size = best_size - 1;   // the clique in G[N(v)]; v completes it
                s.run(deadline, subsh);
                if ((int)s.best.size() < best_size - 1) return;
                s.clear();
                std::fill(s.conf.begin(), s.conf.end(), 1);
                for (int u : s.best)
                    if (s.slack[u] == 0) s.add(u, true);
                if ((int)s.C.size() != best_size - 1) return;
                s.pool_offer();
                s.plateau_walk(1 << 30, deadline);
                // map back to G and re-attach the pinned vertex
                for (auto &c : s.pool) {
                    std::vector<int> full;
                    full.reserve(c.size() + 1);
                    for (int u : c) full.push_back(map[u]);
                    full.push_back(v);
                    found[t].push_back(std::move(full));
                }
                return;
            }
    ...
    //
    // SN83_MULT=1 additionally counts HOW MANY CHAINS independently found each clique
    // and emits the pool in ascending order of that count. The reasoning: a clique only
    // one of our chains reached sits in a small basin of attraction, and a small basin
    // is precisely what makes OTHER miners' searches miss it too. Since diversity pays
    // (1/count) with count including us, a clique nobody else submits is worth 1.0
    // where one we share is worth 0.5 -- so ordering the pool by our own chain
    // multiplicity is a submit-time-available proxy for field rarity, which is
    // otherwise unknowable. Measured oracle selection is worth +0.067 reward.
    int multsort = 0;
    if (const char *e = getenv("SN83_MULT")) multsort = atoi(e);
    ...
    for (int t = 0; t < n_threads; ++t) {
        for (auto &c : found[t]) {
            ...
            for (size_t i2 = 0; i2 < keys.size(); ++i2)
                if (keys[i2] == h) { at = (int)i2; break; }
            if (at >= 0) { ++mult[at]; continue; }
            std::vector<char> in(n, 0);
            std::vector<int> cnt(n, 0);
            ...
            uniq.push_back(c);
            mult.push_back(1);
            // Without mult-sort the old early exit at `want` is kept exactly, so
            // SN83_MULT=0 is a byte-for-byte control.
            if (!multsort && (int)uniq.size() >= want) break;
        }
        if (!multsort && (int)uniq.size() >= want) break;
    }
    if (multsort && uniq.size() > 1) {
        std::vector<int> ord(uniq.size());
        for (size_t i2 = 0; i2 < ord.size(); ++i2) ord[i2] = (int)i2;
        std::stable_sort(ord.begin(), ord.end(),
                         [&](int a, int b) { return mult[a] < mult[b]; });
        std::vector<std::vector<int>> u2;
        u2.reserve(uniq.size());
        for (int i2 : ord) u2.push_back(uniq[i2]);
        uniq.swap(u2);
    }
    if (uniq.empty()) {
