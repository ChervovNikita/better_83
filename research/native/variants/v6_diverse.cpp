...
int sn83_solve(const uint8_t *adj, int n, double time_limit, uint64_t seed,
    ...
    // This variant's one change: each thread gets a DIFFERENT configuration rather
    // than the same one under a different seed.
    //
    // Why: the measured lever here is diversity. Re-running only the tasks the
    // champion loses recovers 32-52% of them per fresh seed and 85% across eight,
    // so the misses are reachable and the search just fails to reach them on a
    // given run. Seeds diversify the trajectory; parameters diversify the SHAPE of
    // the search — how long it exploits before perturbing, how greedily it picks,
    // how much of the incumbent survives a restart. The restart schedule is the
    // axis with the most published disagreement: LSCC uses a fixed 4000 steps, but
    // RRWL and TRSC measure effective periods of ~29k-35k on dense graphs, and AMTS
    // reports that on random instances a long search with NO restart beats frequent
    // restarts (https://leria-info.univ-angers.fr/~jinkao.hao/papers/JOCONov2011.pdf).
    // Rather than pick a winner, span the range across the pool.
    static const int RESTARTS[8] = {500, 1000, 2000, 4000, 8000, 16000, 32000, 64000};
    static const int BMS[4] = {16, 32, 64, 128};
    static const int KEEP[4] = {1, 2, 4, 8};
    ...
    for (int t = 0; t < n_threads; ++t) {
        pool.emplace_back([&, t] {
            Params tp = p;
            tp.restart_steps = RESTARTS[t % 8];
            tp.bms_k = BMS[(t / 8) % 4];
            tp.perturb_keep = KEEP[(t / 2) % 4];
            Search s(g, tp, seed + 0x9e3779b97f4a7c15ULL * (u64)(t + 1));
            s.run(deadline, sh);
            locals[t] = s.best;
