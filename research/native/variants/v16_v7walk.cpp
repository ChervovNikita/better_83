...
//
// v16 = v7 with the WALK perturbation replacing keep-k-random-from-incumbent.
//
// This targets the one thing extra search cannot fix. The GPU feasibility study
// measured that 11 of 30 losses were reached by NONE of 8 independent mid-depth
// chains, and 13 of 19 by none of 50 shallow ones — so for that residual the issue
// is which basins are reachable at all, not how many times we try.
//
// The perturbation is what decides reachability. v7 keeps k random vertices of the
// incumbent and rebuilds around them, which stays in the same region by
// construction. The walk move instead forces in a RANDOM vertex v and keeps only
// C cap N(v), which can land anywhere in the graph.
//
// This is also the empirically tuned rule for our instance class: DLS-MC's penalty
// delay is tuned to pd=1 for every C*.9, gen400* and p_hat*-3 instance, and pd=1
// switches penalties off entirely, leaving exactly plateau search plus this
// perturbation (https://arxiv.org/pdf/1109.5717 Tables 1-2). SCCWalk derives the
...
struct Search {
    ...
    void perturb() {
        ++n_restart;
        if (rnd(10) == 0) {                      // occasional cold restart, as before
            seed_from(rnd(g.n));
            construct();
            return;
        }
        int v = rnd(g.n);                        // walk: force v in, keep C cap N(v)
        std::vector<int> keep;
        keep.reserve(C.size());
        for (int u : C)
