...
//
// v15 = v7 plus HARD COLD RESTARTS that slice the deadline into equal pieces.
//
// This comes straight out of a GPU feasibility study that measured, on this box's
// A4000, that a GPU delivers 15.8x our aggregate step rate (138.9M/s vs 8.8M/s on 8
// cores) — and that it does not matter, because restart diversity SATURATES at
// roughly 24-48 independent chains. Measured on our own loss set with our own
// solver, one core, 5.28 s:
//
//     1 deep chain of 5.28 s      -> reaches the field's answer on 10/30
//     best-of-8 chains of 1.78 s  -> reaches it on 19/30
//
// Same total CPU. The depth beyond ~1.8 s is being wasted, and 8 threads x 3 slices
// is 24 chains — i.e. the saturation point is already inside our existing budget.
//
// Note this is NOT what our perturbation does. perturb() keeps part of the
// incumbent and rebuilds around it, which stays in the same region; a cold restart
// re-seeds from a random vertex with conf cleared, and `best` is kept separately so
// nothing found so far is lost.
//
// SN83_SLICES sets the number of slices (default 3; 1 restores v7 exactly).
//
...
struct Search {
    ...
    void run(Clock::time_point deadline, Shared &sh) { run_sliced(deadline, sh, 1); }

    // Split the remaining time into `slices` and cold-restart at each boundary.
    void run_sliced(Clock::time_point deadline, Shared &sh, int slices) {
        Clock::time_point t0 = Clock::now();
        auto total = deadline - t0;
        seed_from(rnd(g.n));
        construct();
        best = C;
        long long last_gain = 0;
        int slice = 1;
        Clock::time_point next_cut = slices > 1 ? t0 + total / slices : deadline;
        ...
        for (;;) {
            if ((step & 63) == 0) {
                ...
                if (now >= next_cut) {                  // slice boundary: start over cold
                    ++slice;
                    next_cut = (slice >= slices) ? deadline : t0 + (total * slice) / slices;
                    seed_from(rnd(g.n));
                    construct();
                    if (k > (int)best.size()) { best = C; record(sh); }
                    last_gain = step;
                    continue;
                }
            }
...
int sn83_solve(const uint8_t *adj, int n, double time_limit, uint64_t seed,
    ...
    int slices = 3;                                     // measured saturation: ~24 chains
    if (const char *e = getenv("SN83_SLICES")) slices = std::max(1, atoi(e));
    Shared sh;
    ...
    for (int t = 0; t < n_threads; ++t) {
        pool.emplace_back([&, t] {
            ...
            s.run_sliced(deadline, sh, slices);
            locals[t] = s.best;
