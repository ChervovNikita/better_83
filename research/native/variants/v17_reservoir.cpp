...
//
// v17 = v7 that COLLECTS distinct maximum cliques and returns a random one.
//
...
struct Search {
    ...
    static u64 clique_key(const std::vector<int> &c) {
        u64 h = 1469598103934665603ULL;             // order-independent: XOR of mixes
        for (int v : c) {
            u64 x = (u64)v * 0x9e3779b97f4a7c15ULL;
            x ^= x >> 29;
    ...
    void pool_offer(const std::vector<int> &c) {
        if (pool_cap <= 0) return;
        u64 key = clique_key(c);
        for (u64 k2 : pool_keys)
            if (k2 == key) return;                  // already have this one
        if ((int)pool.size() < pool_cap) {
            pool.push_back(c);
            pool_keys.push_back(key);
        } else {                                    // reservoir sampling
            int j = rnd(pool_cap * 2);
            if (j < pool_cap) { pool[j] = c; pool_keys[j] = key; }
        }
    }
    ...
    void run(Clock::time_point deadline, Shared &sh) {
        ...
        for (;;) {
            ...
            if ((step & 255) == 0 && k == (int)best.size() && k > 0) pool_offer(C);
            if (!swapBuf.empty()) {
                ++n_swap;
                int v = pick(swapBuf);
                int u = blocker(v);
